...
readonly PROFILE_NAME="Neo-Light.profile"
readonly SCHEME_NAME="Neo-Light.colorscheme"
readonly DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/konsole"
readonly CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}"
readonly CONFIG_FILE="$CONFIG_DIR/konsolerc"
readonly STATE_DIR="$DATA_DIR/.neo-light"
...
set_default_profile() {
    local value="$1"
    if [[ "$value" == __NEO_LIGHT_MISSING__ ]] && command -v kwriteconfig6 >/dev/null 2>&1; then
        kwriteconfig6 --file "$CONFIG_FILE" --group 'Desktop Entry' \
            --key DefaultProfile --delete
    elif [[ "$value" == __NEO_LIGHT_MISSING__ ]] && command -v kwriteconfig5 >/dev/null 2>&1; then
        ...
        kwriteconfig5 --file "$CONFIG_FILE" --group 'Desktop Entry' \
            --key DefaultProfile "$value"
    elif [[ "$value" == __NEO_LIGHT_MISSING__ ]]; then
...
if [[ ! -d "$STATE_DIR" ]]; then
    printf 'Neo-Light install state not found; nothing to uninstall.\n'
    exit 0
fi
...
if [[ -f "$STATE_FILE" ]]; then
    ...
    if [[ "$state_type" == file-missing ]]; then
        set_default_profile __NEO_LIGHT_MISSING__
...
printf 'Uninstalled Neo-Light and restored the previous default Profile.\n'
